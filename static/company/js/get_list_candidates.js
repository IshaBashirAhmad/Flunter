...
var search_data = [];
var loader = document.getElementById('table-loader');
let relatedUsers = [];
let pageNumber = 1;
let requiredDataURL = `/save/list/${list_id}/get_profiles?perPage=20`;
...
async function getSharedUserList() {
    ...
    response.json().then(function(res) {
        ...
        document.querySelector('.list-name').innerText = sessionStorage.getItem('listName') || 'List';
        getRecords(requiredDataURL);
    })
}

window.addEventListener('DOMContentLoaded', getSharedUserList);
...
function generatePages(currentPage, totalPages, previousLink, nextLink) {
    ...
    pagesContainer.querySelectorAll('span').forEach((span) => {
        if ((!span.classList.contains('active'))  && (!span.classList.contains('ellipsis-container'))) {
            ...
            span.setAttribute("onclick", `getRecords('${pageUrl}')`);
        }
    })
    if (previousLink) {
        let firstPageURL = setParams(requiredDataURL, 'page', 1);
        getFirstPageBtn.setAttribute('onclick', `getRecords('${firstPageURL}')`);
        getFirstPageBtn.classList.remove('opacity-point-3-5');
        getFirstPageBtn.classList.add('cursor-pointer');
        getPreviousPageBtn.setAttribute('onclick', `getRecords('${previousLink}')`);
    ...
    if (nextLink) {
        ...
        getLastPageBtn.setAttribute('onclick', `getRecords('${lastPageURL}')`);
        getLastPageBtn.classList.remove('opacity-point-3-5');
        getLastPageBtn.classList.add('cursor-pointer');
        getNextPageBtn.setAttribute('onclick', `getRecords('${nextLink}')`);
