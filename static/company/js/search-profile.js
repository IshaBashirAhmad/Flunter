...
let keywordInput = document.querySelector('input[name="keywords"]');
...
let requiredDataURL = '/candidate/profiles?perPage=20&ordering=-percentage,-id';

const companySizeCheckboxes = document.querySelectorAll('input[name="company_size_filter"]');
const contactDetailsCheckboxes = document.querySelectorAll('input[name="contact_details"]');
const seniorityLevelCheckboxes = document.querySelectorAll('input[name="seniority_level"]');

let filtersForm = document.getElementById('filters-form');
let contactNameFilterInput = document.getElementById('contact-name');
let searchKeywords = {
    "keyword": '', "city": [], "state": [], "region": [], "country": [], "current_position": [], 
    "person_skills": [], "company_name": [], "employee_headcount": [],
    "condition": '', "contact_options": [], "full_name": '', 'seniority': []
};
...
function initializeSearchFromURL() {
    ...
    if (queryString != '') {
        ...
        selectedCityList = (urlParams.get('city') || '').split(',').filter(Boolean);
        selectedStateList = (urlParams.get('state') || '').split(',').filter(Boolean);
        selectedRegionList = (urlParams.get('region') || '').split(',').filter(Boolean);
        selectedCountryList = (urlParams.get('country') || '').split(',').filter(Boolean);

        selectedCityList.forEach((city) => {
            const newTag = createLocationTag(city, 'city');
            locationTags.push(newTag);
        })

        selectedStateList.forEach((state) => {
            const newTag = createLocationTag(state, 'state');
            locationTags.push(newTag);
        })

        selectedRegionList.forEach((region) => {
            const newTag = createLocationTag(region, 'region');
            locationTags.push(newTag);
        })

        selectedCountryList.forEach((country) => {
            const newTag = createLocationTag(country, 'country');
            locationTags.push(newTag);
        })
        ...
        let jobs_title_list = urlParams.get('current_position').split(',') || [];
        let skills_list = urlParams.get('person_skills').split(',') || [];
        let company_name_list = urlParams.get('company_name').split(',') || [];

        jobs_title_list.forEach((job_title) => {
            document.getElementById('job-tag-input').value = job_title;
            document.getElementById('job-tag-input').dispatchEvent(customEvent);
        })
        skills_list.forEach((skill) => {
            document.getElementById('tag-input').value = skill;
            document.getElementById('tag-input').dispatchEvent(customEvent);
        })
        company_name_list.forEach((company_name) => {
            document.getElementById('company-tag-input').value = company_name;
            document.getElementById('company-tag-input').dispatchEvent(customEvent);
        })

        
        const company_size_ranges = urlParams.get('employee_headcount').split(',') || [];
        companySizeCheckboxes.forEach(checkbox => {
            checkbox.checked = company_size_ranges.includes(checkbox.value);
        });
        
        
        const contact_detail_options = urlParams.get('contact_options').split(',') || [];
        contactDetailsCheckboxes.forEach(checkbox => {
            checkbox.checked = contact_detail_options.includes(checkbox.value);
        });
        
        seniorityLevelCheckboxes.forEach(checkbox => {
            checkbox.checked = urlParams.get('seniority').includes(checkbox.value);
        });
        
        document.getElementById('contact-name').value = urlParams.get('full_name') || '';
        document.querySelector(`input[name="contact_details_radio"][value="${urlParams.get('condition') || 'or'}"]`).checked = true;
...
function createLocationTag(text, type) {
    ...
    removeBtn.innerHTML = `<svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7.5 2.5L2.5 7.5M2.5 2.5L7.5 7.5" stroke="#392680" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                            `
...
function closeLocationContainers(event) {
    if (!(locationInput.contains(event.target)) && !(locationDropdown.contains(event.target)) && !(locationTagContainer.contains(event.target)) && !(locationSelectionContainer.contains(event.target)) && !(locationDropdown.classList.contains('hide'))) {
        ...
        // locationTagContainer.classList.add('hide');
    }
}

document.body.addEventListener('click', closeLocationContainers);
...
function getSearchDataFromStorage() {
    let sessionData = sessionStorage.getItem('searchData');
    sessionData = JSON.parse(sessionData);
    if (sessionData) {
        keywordInput.value = sessionData.keywords;
        
        locationTagContainer.innerHTML = '';
        locationTags = [];
        
        selectedCityList = sessionData.city || [];
        selectedStateList = sessionData.state || [];
        selectedRegionList = sessionData.region || [];
        selectedCountryList = sessionData.country || [];
        ...
        let jobs_title_list = sessionData.job_title || [];
        let skills_list = sessionData.skills || [];
        let company_name_list = sessionData.companies || [];
        ...
        companySizeCheckboxes.forEach(checkbox => {
            checkbox.checked = sessionData.head_count.includes(checkbox.value);
        });
        
        let contact_option = sessionData.search_via || [];
        contactDetailsCheckboxes.forEach(checkbox => {
            checkbox.checked = contact_option.includes(checkbox.value);
        });
        
        seniorityLevelCheckboxes.forEach(checkbox => {
            checkbox.checked = sessionData.seniority.includes(checkbox.value);
        });
        
        document.querySelector(`input[name="contact_details_radio"][value="${sessionData.search_via_op || 'or'}"]`).checked = true;
        contactNameFilterInput.value = sessionData.contact_name || '';

        // let filterData = getFilterData();
        // searchKeywords = {
        //     "keywords": keywordInput.value, "location": [...selectedLocations], "job_titles": [...filterData.jobs_title_list],
        //     "skills": [...filterData.skills_list], "company_names": [...filterData.company_name_list],
        //     "employee_headcount": [...company_size_ranges], "contact_operation": filterData.contact_details_radio,
        //     "contact_options": [...filterData.contact_details], "contact_name": contactNameFilterInput.value,
        //     "seniority_levels": [...seniority_levels]
        // };
        
        // searchId = sessionData.id;
        // saveSearchId = sessionData.id;
        document.getElementById('search-result-btn').click();
        sessionStorage.removeItem('searchData');
    }
}

window.addEventListener('load', getSearchDataFromStorage);
...
function showProgress() {
    document.getElementById('save-result-btn').classList.add('opacity-point-2');
...
function hideProgress(){
    document.getElementById('save-result-btn').classList.remove('opacity-point-2');
...
async function searchForm(event) {
    ...
    let searchLocationList = locationTags.map((tag) => tag.querySelector('span').innerText);
    searchLocationList = searchLocationList.filter(item => item.trim() !== '');
    if (userData.user.sub == null) {
        errorMsg.innerText = 'Please select your plan to proceed.';
        errorMsg.classList.add('active');
        return false;
    }
    if (keywordInput.value.trim().length == '' && selectedCityList.length == 0 && selectedStateList.length == 0 && selectedRegionList.length == 0 && selectedCountryList.length == 0) {
        errorMsg.innerText = 'Please enter keyword or location to proceed.';
        errorMsg.classList.add('active');
        return false;
    }

    let filterData = getFilterData();
    let keywordValue = removeExtraSpaces(keywordInput.value);
    
    if (searchKeywords['keyword'] !== keywordValue || !arraysEqualIgnoreOrder(searchKeywords['city'], [...selectedCityList]) || !arraysEqualIgnoreOrder(searchKeywords['state'], [...selectedStateList]) || !arraysEqualIgnoreOrder(searchKeywords['region'], [...selectedRegionList]) || !arraysEqualIgnoreOrder(searchKeywords['country'], [...selectedCountryList]) || !arraysEqualIgnoreOrder(searchKeywords['current_position'], [...filterData.jobs_title_list]) || !arraysEqualIgnoreOrder(searchKeywords['person_skills'], [...filterData.skills_list]) || !arraysEqualIgnoreOrder(searchKeywords['company_name'], [...filterData.company_name_list]) || !arraysEqualIgnoreOrder(searchKeywords['employee_headcount'], [...filterData.company_size_ranges]) || searchKeywords['condition'] != filterData.contact_details_radio || !arraysEqualIgnoreOrder(searchKeywords['contact_options'], [...filterData.contact_details]) || searchKeywords['full_name'] != contactNameFilterInput.value || !arraysEqualIgnoreOrder(searchKeywords['seniority'], [...filterData.seniority_level_ranges])) {
        searchKeywords = {
            "keyword": keywordValue, "city": [...selectedCityList], "state": [...selectedStateList],
            "region": [...selectedRegionList], "country": [...selectedCountryList], "current_position": [...filterData.jobs_title_list],
            "person_skills": [...filterData.skills_list], "company_name": [...filterData.company_name_list],
            "employee_headcount": [...filterData.company_size_ranges], "condition": filterData.contact_details_radio,
            "contact_options": [...filterData.contact_details], "full_name": contactNameFilterInput.value,
            'seniority': [...filterData.seniority_level_ranges]
        };
        searchId = null;
        saveSearchId = null;
    }
    ...
    requiredDataURL = setParams(requiredDataURL, 'keyword', searchKeywords.keyword);
    requiredDataURL = setParams(requiredDataURL, 'city', selectedCityList.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'state', selectedStateList.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'region', selectedRegionList.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'country', selectedCountryList.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'current_position', filterData.jobs_title_list.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'person_skills', filterData.skills_list.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'company_name', filterData.company_name_list.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'employee_headcount', filterData.company_size_ranges.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'seniority', filterData.seniority_level_ranges.join(',') || '');
    requiredDataURL = setParams(requiredDataURL, 'condition', filterData.contact_details_radio || 'or');
    requiredDataURL = setParams(requiredDataURL, 'full_name', filterData.contact_name || '');
    
    for (const [key, checkbox] of Object.entries(filterData.contact_options))
        requiredDataURL = setParams(requiredDataURL, key, checkbox.checked);
    ...
    try {
        ...
        if (response.headers.get('content-type').includes('application/json')) {
            ...
            if (response.status == 200) {
                ...
                document.getElementById('start-record').innerText = pageStartEndRecords.start;
                document.getElementById('end-record').innerText = pageStartEndRecords.end;
                ...
                const currentState = {
                    ...
                    paginationContainerHTML: paginationContainer.innerHTML,
...
function initializeState() {
    const currentState = {
        ...
        searchData: {
            "keyword": '', "city": [], "state": [], "region": [], "country": [], "current_position": [], 
            "person_skills": [], "company_name": [], "employee_headcount": [], "condition": '', 
            "contact_options": [], "full_name": '', 'seniority': [], "page": 1
        }
    };
...
window.addEventListener('popstate', function(event) {
    if (event.state) {
        ...
        saveSearchId = event.state.saveSearchId;
        detailsContainer.innerHTML = event.state.profileCardHTML;
        paginationContainer.innerHTML = event.state.paginationContainerHTML;
        headerRecordCountText.innerText = event.state.recordCount || 0;
        keywordInput.value = event.state.searchData.keyword || '';
        ...
        selectedCityList = [];
        event.state.searchData.city.forEach((city) => {
            const newTag = createLocationTag(city, 'city');
            locationTags.push(newTag);
        ...
        selectedStateList = [];
        event.state.searchData.state.forEach((state) => {
            const newTag = createLocationTag(state, 'state');
            locationTags.push(newTag);
            selectedStateList.push(state);
        })

        selectedRegionList = [];
        event.state.searchData.region.forEach((region) => {
            const newTag = createLocationTag(region, 'region');
            locationTags.push(newTag);
            selectedRegionList.push(region);
        })

        selectedCountryList = [];
        event.state.searchData.country.forEach((country) => {
            const newTag = createLocationTag(country, 'country');
            locationTags.push(newTag);
            selectedCountryList.push(country);
        })

        event.state.searchData.current_position.forEach((job_title) => {
            document.getElementById('job-tag-input').value = job_title;
            document.getElementById('job-tag-input').dispatchEvent(customEvent);
        })
        event.state.searchData.person_skills.forEach((skill) => {
            document.getElementById('tag-input').value = skill;
            document.getElementById('tag-input').dispatchEvent(customEvent);
        })
        event.state.searchData.company_name.forEach((company_name) => {
            document.getElementById('company-tag-input').value = company_name;
            document.getElementById('company-tag-input').dispatchEvent(customEvent);
        })

        companySizeCheckboxes.forEach(checkbox => {
            checkbox.checked = event.state.searchData.employee_headcount.includes(checkbox.value);
        });

        contactDetailsCheckboxes.forEach(checkbox => {
            checkbox.checked = event.state.searchData.contact_options.includes(checkbox.value);
        });

        seniorityLevelCheckboxes.forEach(checkbox => {
            checkbox.checked = event.state.searchData.seniority.includes(checkbox.value);
        });

        document.getElementById('contact-name').value = event.state.searchData.full_name || '';
        document.querySelector(`input[name="contact_details_radio"][value="${event.state.searchData.condition || 'or'}"]`).checked = true;
...
function scrollToTop() {
    let firstResultItem = document.getElementById('result-list').firstChild;
    if (firstResultItem)
        firstResultItem.scrollIntoView();
}


async function createSearch(data, record_count=0) {
    try {
        let searchFilterData = {
            "keywords": data.keyword, "state": data.state, "city": data.city, "region": data.region,
            "country": data.country, "search_via": data.contact_options, "job_title": data.current_position, 
            "skills": data.person_skills, "companies": data.company_name, "head_count": data.employee_headcount, 
            "search_via_op": data.condition, "contact_name": data.full_name, "number_of_results": record_count, 
            "seniority": data.seniority, "search_type": 'recent' 
        };
        ...
        let response = await requestAPI(`${apiURL}/save/search`, JSON.stringify(searchFilterData), headers, "POST");
        response.json().then(function(res) {
...
async function saveSearchForm(event){
    event.preventDefault();
    let form = document.querySelector("#searchForm");
    button = document.querySelector('#save-result-btn')
    ...
    try {
        ...
        let response = await requestAPI(`${apiURL}/save/search/${searchId}`, JSON.stringify({"search_type": "saved"}), headers, "PATCH");
        if (response.headers.get('content-type').includes('application/json')) {
            let res = await response.json();
            if (response.status == 200) {
...
function searchCompanyRecords(companyName) {
    ...
    triggerEvent(seniority_clear_btn, 'click');
    triggerEvent(document.getElementById('clear-skills-tags-button'), 'click');
    triggerEvent(document.getElementById('clear-jobs-tags-button'), 'click');
    triggerEvent(document.getElementById('clear-company-tags-button'), 'click');
...
function generatePages(currentPage, totalPages, previousLink, nextLink) {
    ...
    pagesContainer.querySelectorAll('span').forEach((span) => {
        if ((!span.classList.contains('active'))  && (!span.classList.contains('ellipsis-container'))) {
            ...
            span.setAttribute("onclick", `setPage(${page}); getRecords('${pageUrl}')`);
        }
    })
    if (previousLink) {
        ...
        getPreviousPageBtn.setAttribute('onclick', `setPage(${currentPage - 1}); getRecords('${previousLink}')`);
    ...
    if (nextLink) {
        ...
        getNextPageBtn.setAttribute('onclick', `setPage(${currentPage + 1}); getRecords('${nextLink}')`);
...
function openSuccessModal(modalId) {
    ...
    modal.querySelector('#success-statement').innerText = 'This search has been saved successfully. You can access it anytime from your saved searches.';
